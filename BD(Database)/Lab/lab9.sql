...
alter table angajati_rs
add constraint fk_ang_depart_rs foreign key(cod_dep) references departamente_rs(cod_dep);

select constraint_name, constraint_type, table_name
from user_constraints
where lower(table_name) = 'angajati_rs';

drop table angajati_rs;

create table angajati_rs(
    cod_ang number(4) constraint pk_ang_rs primary key,
    nume varchar(20) constraint null_name_rs not null,
    prenume varchar(20),
    email char(15) constraint unq_email_rs unique,
    data_ang  date  default sysdate,
    job  varchar2(10),
    cod_sef  number(4) constraint fk_ang_ang_rs references angajati_rs(cod_ang),
    salariu  number(8, 2),
    cod_dep number(2) constraint ck_cod_dep_rs check(cod_dep>0),
    comision number(4, 2),
    constraint unq_nume_pren_rs unique(nume, prenume),
    constraint ck_sal_com_rs check(salariu>comision*100),
    constraint fk_ang_depart_rs foreign key(cod_dep) references departamente_rs(cod_dep));
    
insert into angajati_rs
select * from angajati_ong;

select * from angajati_rs;
commit;

alter table angajati_rs
drop constraint fk_ang_depart_rs;

alter table angajati_rs
add constraint fk_ang_depart2_rs foreign key(cod_dep) 
        references departamente_rs(cod_dep) on delete cascade;

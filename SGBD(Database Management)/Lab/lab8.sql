...
CREATE OR REPLACE FUNCTION f2_rs(nume_ang employees.last_name%TYPE)
...
SELECT f2_rs('K') FROM dual;

VAR sal NUMBER
EXECUTE :sal := f2_rs('Bell');
PRINT sal
...
CREATE OR REPLACE PROCEDURE p4_rs(nume_ang employees.last_name%TYPE)
IS
    salariu employees.salary%TYPE;
BEGIN
    SELECT salary INTO salariu
...
CREATE OR REPLACE PROCEDURE p5_rs(nr IN OUT employees.employee_id%TYPE)
IS
BEGIN
...
CREATE OR REPLACE FUNCTION factorial_rs(n NUMBER)
RETURN NUMBER
IS
BEGIN
    IF n = 0 THEN
        RETURN 1;
    ELSE 
        RETURN n * factorial_rs(n - 1);
    END IF;
END;
/

BEGIN
    dbms_output.put_line(factorial_rs(5));
END;
/
...
CREATE OR REPLACE FUNCTION medie_rs
RETURN NUMBER
IS  
    rezultat NUMBER;
BEGIN
